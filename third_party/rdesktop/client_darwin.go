...
func (cli *osBase) Size() (image.Point, error) {
	width := C.CGDisplayPixelsWide(cli.id)
	height := C.CGDisplayPixelsHigh(cli.id)
	return image.Point{
		X: int(width),
		Y: int(height),
	}, nil
...
func (cli *Client) screenshot(img *image.RGBA) error {
	if C.screenshot((*C.uchar)(unsafe.Pointer(&img.Pix[0])), C.int(img.Rect.Dx()), C.int(img.Rect.Dy()), C.bool(cli.showCursor)) != 0 {
		return fmt.Errorf("screen capture failed; enable Screen Recording permission for ChickReomte in System Settings")
	}
	return nil
}
